...
public abstract class LumineMod {
    ...
    final void terminate() throws IllegalStateException {
        ...
        try {
            ...
            Lumine.warn("Server mod for " + modId + " is already disabled on termination.");
        }
        try {
            if (clientMod != null) {
                clientMod.terminate();
            ...
            Lumine.warn("Client mod for " + modId + " is already disabled on termination.");
        }
        stop();
        enabled = false;
    }
    ...
    static String getValidId(ModConfiguration config) {
        if (config == null) {
            Lumine.LOGGER.error("Exception while retrieving valid id", new IllegalStateException("Configuration must be set to retrieve id"));
            return "";
        }
